...
# Canonical 15-min acceptance run for action-chunk diffusion student baseline.
# Usage:
#   scripts/screwdriver_student_diffusion_action_chunk_15min_docker.sh GPU_ID SEED TEACHER_CACHE [WINDOW_SEC] [DIFF_CACHE]

GPU_ID=${1:-0}
SEED=${2:-42}
TEACHER_CACHE=${3:?Missing TEACHER_CACHE, e.g. run_a}
WINDOW_SEC=${4:-900}
DIFF_CACHE=${5:-${TEACHER_CACHE}}
...
OUT_DIR="outputs/XHandHoraScrewDriver_student_diffusion_action_chunk/${DIFF_CACHE}"
...
PY
set +e
timeout ${WINDOW_SEC} scripts/screwdriver_student_diffusion_action_chunk.sh ${GPU_ID} ${SEED} ${DIFF_CACHE} \
...
echo "Diffusion action-chunk 15-min acceptance finished."
