...
# Canonical 15-min acceptance run for ProprioAdapt train-range ablation.
# Usage:
#   scripts/screwdriver_student_padapt_trainrange_15min_docker.sh GPU_ID SEED TEACHER_CACHE [WINDOW_SEC] [ABLATION_CACHE]

GPU_ID=${1:-0}
SEED=${2:-42}
TEACHER_CACHE=${3:?Missing TEACHER_CACHE, e.g. run_a}
WINDOW_SEC=${4:-900}
ABLATION_CACHE=${5:-${TEACHER_CACHE}}
...
OUT_DIR="outputs/XHandHoraScrewDriver_student_padapt_trainrange/${ABLATION_CACHE}"
...
PY
set +e
timeout ${WINDOW_SEC} scripts/screwdriver_student_padapt_trainrange.sh ${GPU_ID} ${SEED} ${ABLATION_CACHE} \
...
echo "ProprioAdapt train-range ablation 15-min acceptance finished."
echo "Teacher cache: ${TEACHER_CACHE}"
echo "Ablation cache: ${ABLATION_CACHE}"
