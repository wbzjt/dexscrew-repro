...
# Canonical 15-min acceptance run for PureBC student baseline.
# Usage:
#   scripts/screwdriver_student_purebc_15min_docker.sh GPU_ID SEED TEACHER_CACHE [WINDOW_SEC] [PUREBC_CACHE]

GPU_ID=${1:-0}
SEED=${2:-42}
TEACHER_CACHE=${3:?Missing TEACHER_CACHE, e.g. run_a}
WINDOW_SEC=${4:-900}
PUREBC_CACHE=${5:-${TEACHER_CACHE}}
...
OUT_DIR="outputs/XHandHoraScrewDriver_student_purebc/${PUREBC_CACHE}"
...
PY
set +e
timeout ${WINDOW_SEC} scripts/screwdriver_student_purebc.sh ${GPU_ID} ${SEED} ${PUREBC_CACHE} \
...
echo "PureBC 15-min acceptance finished."
echo "Teacher cache: ${TEACHER_CACHE}"
echo "PureBC cache: ${PUREBC_CACHE}"
