...
# Continue the four CoDrive diffusion-class students from the completed 1h
# checkpoints. This keeps the PPO teacher/task fixed and uses each algorithm's
# own previous student checkpoint as the resume point.
...
BASE_RUN="${BASE_RUN:-codrive_diffusion4_s42_1h_20260430_085750}"
RUN_TAG="${RUN_TAG:-codrive_diffusion4_s${SEED}_continue2h_from_1h_$(date +%Y%m%d_%H%M%S)}"
PIPE_ROOT="${PIPE_ROOT:-outputs/cloud_pipeline_codrive_diffusion4_continue2h}"
...
resume_ckpt() {
  case "$1" in
    diffusion_latent)
      echo "outputs/Dexh13HoraLightbulb_student_diffusion_latent_codrive/${BASE_RUN}/stage2_diffusion_nn/model_best.ckpt"
      ;;
    consistency_latent)
      echo "outputs/Dexh13HoraLightbulb_student_consistency_codrive/${BASE_RUN}/stage2_consistency_nn/model_best.ckpt"
      ;;
    flow_matching_latent)
      echo "outputs/Dexh13HoraLightbulb_student_flow_matching_codrive/${BASE_RUN}/stage2_flow_nn/model_best.ckpt"
      ;;
    diffusion_action_chunk)
      echo "outputs/Dexh13HoraLightbulb_student_diffusion_action_chunk_codrive/${BASE_RUN}/stage2_diffusion_action_chunk_nn/model_best.ckpt"
      ;;
    *) return 2 ;;
  esac
}
...
run_one() {
  local label="$1"
  local class output resume log status_file
  class="$(algo_class "${label}")"
  output="$(output_name "${label}")"
  resume="$(resume_ckpt "${label}")"
  log="${PIPE_DIR}/logs/${label}.log"
  status_file="${PIPE_DIR}/${label}.status"

  if [[ ! -f "${resume}" ]]; then
    echo "[${label}] missing resume checkpoint: ${resume}" >&2
    echo "2" > "${status_file}"
    return 2
  fi
  ...
  cmd=(
    ...
    "experiment=student_codrive_${label}_continue2h"
    "checkpoint=${resume}"
    "${extra[@]}"
  )
...

{
  ...
  echo "[pipeline] base_run=${BASE_RUN}"
...
printf 'label\tstatus\tbest_reward\tresume_ckpt\tcheckpoint_dir\toutput_dir\n' > "${summary}"
for label in "${labels[@]}"; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\toutputs/%s\n' \
    "${label}" "${status}" "${best:-NA}" "$(resume_ckpt "${label}")" "${checkpoint_dir}" "${output}" >> "${summary}"
done
