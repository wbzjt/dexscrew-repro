...
class BCStudent:
    ...
    def _collect_demos(self, num_steps: int):
        ...
        - 不需要 state=[obs, extrin] 向量化（BCStudent 从 buffer 采样时会重新通过 adapt_tconv
          + actor_mlp 做 forward）。
        - 同时更新 sa_mean_std / running_mean_std（需要代表 teacher demo 分布）。
        ...
        # demo 收集期间正常更新归一化统计；buffer 里存的是 RAW（未归一化）数据
        self.running_mean_std.train()
        self.sa_mean_std.train()
        if self.normalize_priv:
    ...
    def _save_demo_buffer(self):
        try:
            meta = {
                ...
                # running_mean_std 在 teacher ckpt 里一定有，stage1 → stage2 会稳定继承。
                # 但 demo 采集阶段会继续更新，保留一份确保 reuse 路径和首次 run 完全一致。
    ...
    def _periodic_student_eval(self):
        ...
        if np.isfinite(mean_r):
            ...
            if mean_r > self.best_eval_rewards:
                ...
                cprint(
                    f"[BC] new best student eval reward = {mean_r:.2f} (saved model_best)",
                    "green", attrs=["bold"],
                )
            self.writer.add_scalar(
