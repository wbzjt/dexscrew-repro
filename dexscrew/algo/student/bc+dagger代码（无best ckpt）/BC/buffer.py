...
class BCBuffer:
    ...
    def save(self, path: str, meta: Optional[dict] = None):
        payload = {
            "obs": self.obs[: self.size].detach().cpu(),
            "proprio_hist": self.proprio_hist[: self.size].detach().cpu(),
            "teacher_action": self.teacher_action[: self.size].detach().cpu(),
            "teacher_extrin": self.teacher_extrin[: self.size].detach().cpu(),
            "size": int(self.size),
        ...
        if self.store_priv_info and self.priv_info is not None:
            payload["priv_info"] = self.priv_info[: self.size].detach().cpu()
        if self.store_point_cloud and self.point_cloud_info is not None:
            payload["point_cloud_info"] = self.point_cloud_info[: self.size].detach().cpu()
