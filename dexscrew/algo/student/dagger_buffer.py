"""
DAgger: Dataset Aggregation buffer for student adaptation.

与 DOTPG/RawExpertBuffer 同类的原始观测缓冲区，但只存 DAgger 监督
学习需要的字段（(obs, priv_info, proprio_hist, point_cloud_info,
teacher_action, teacher_extrin)），不存 reward / next_obs / done。

存储方式与 DOTPG 的 RawExpertBuffer 保持一致：
- 默认存 CPU 以便大容量聚合
- dtype 可选 float16 / float32
- 采样时 to(device, return_dtype)，供 student 监督学习使用
"""
...
class DAggerBuffer:
    """DAgger 聚合数据集（ring buffer，存 raw 输入 + teacher 标签）。

    与 ProprioAdapt 不同，本 buffer 只存 teacher 对 student-visited 状态的
    监督标签；训练时由 student 当前的 adapt_tconv / actor_mlp 计算预测并
    与 teacher 标签对齐（latent + action BC）。
    """
    ...
    def __init__(
        ...
        store_priv_info: bool = True,
        store_point_cloud: bool = True,
    ):
    ...
    def sample(self, batch_size: int):
        ...
        and optionally priv_info / point_cloud_info when stored.
        """
        assert self.size > 0, "DAggerBuffer is empty"
