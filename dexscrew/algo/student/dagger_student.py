...
from dexscrew.algo.student.dagger_buffer import DAggerBuffer
...
class DAggerStudent(EvalSelectMixin):
    ...
    def _supervised_update(self):
        ...
        Loss:
          ...
          bc_loss     = || a_student_raw - clamp(a_teacher) ||^2
          adapt_loss  = latent_coef * latent_loss + action_coef * bc_loss
        ...
        # --- BC loss ---
        # Do not clamp the student action before MSE.  In this task many teacher
        # dimensions sit near +/-1; pre-MSE clamp would zero the gradient exactly
        # where the adapter needs a correction signal.  The executed action is
        # still clamped in rollout, and the teacher target is clamped here.
        ...
        # makes the action term about actions_num times weaker and can leave
        # DAgger dominated by latent matching while pure-student actions remain poor.
    ...
    def _fixed_step_eval(self, action_fn, label: str, num_steps: int):
        ...
        cprint(
            f"[DAgger][{label}] EvalSummary steps={steps} "
    ...
    def train(self):
        ...
        while self.agent_steps <= max_agent_steps:
            ...
            if eval_select_metrics is not None:
                next_obs = (
                    eval_select_metrics.get("final_obs_dict", None)
                    if isinstance(eval_select_metrics, dict)
                    else None
                )
                if next_obs is not None:
                    obs_dict = next_obs
